...
function elementID(v) { return(document.getElementById(v)); }
function elementIDstyle(v) { return(elementID(v).style); }
function browser(v) { return(Math.max(navigator.userAgent.toLowerCase().indexOf(v),0)); }
function toggle(v) { elementIDstyle(v).display=(elementIDstyle(v).display=='none'?'block':'none'); }
...
function HSVslide(d,o,e) {
	...
	function drag(e) { if(!stop) {
		...
		if(d=='SVslide') { tXY(e); ds.left=ckHSV(tX-oo,wSV)+'px'; ds.top=ckHSV(tY-oo,wSV)+'px';
			...
			mkColor(commit()); elementIDstyle('SV').backgroundColor='#'+hsv2hex(Array(HSV[0],100,100));
	...
	if(stop) { stop=''; var ds=elementIDstyle(d!='drag'?d:o);

		if(d=='drag') { var oX=parseInt(ds.left), oY=parseInt(ds.top), eX=XY(e), eY=XY(e,1); zindex(elementID(o)); }
		else { var left=(elementID(o).offsetLeft+10), top=(elementID(o).offsetTop+22), tX, tY, oo=(d=='Hslide')?2:4; if(d=='SVslide') slideHSV[0]=HSV[0]; }
...
function HSVupdate(v) { HSV=v?hex2hsv(v):Array(slideHSV[0],slideHSV[1],slideHSV[2]);
	...
	mkColor(v); elementID('plugHEX').innerHTML=v; return(v);

}

function loadSV() { var z=''; for(var i=SVHeight; i>=0; i--) z+="<div style=\"BACKGROUND: #"+hsv2hex(Array(Math.round((359/SVHeight)*i),100,100))+";\"><br /><\/div>"; elementID('Hmodel').innerHTML=z; }

function updateH(v) { HSV=hex2hsv(v);

	elementIDstyle('SV').backgroundColor='#'+hsv2hex(Array(HSV[0],100,100)); 
	elementIDstyle('SVslide').top=(parseInt(wSV-wSV*(HSV[2]/100))-4)+'px'; elementIDstyle('SVslide').left=parseInt(wSV*(HSV[1]/100))+'px';
	elementIDstyle('Hslide').top=(parseInt(wH*((maxValue['h']-HSV[0])/maxValue['h']))-7)+'px';
